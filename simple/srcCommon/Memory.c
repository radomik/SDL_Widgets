...
#define MM_fill_info(mm_fname, alloc_fname, ptr, file, line, __size)	\
{																		\
...
char *wgetcwd(char *buf, size_t size, const char *_file_, int _line_) {
	char *ptr = getcwd(buf, size);
	if (! ptr) { // cannot allocate desired memory
		fprintf(stderr, "wgetcwd: Failed to get current working directory buf=%p, size=%lu (getcwd returned NULL), file: %s, line: %d\n", 
			buf, size, _file_, _line_);
		return ptr;
	}
	else {
		if (! buf) 
			MM_fill_info("wgetcwd", "getcwd", ptr, _file_, _line_, strlen(ptr)+1);
	}
	return ptr;
}
