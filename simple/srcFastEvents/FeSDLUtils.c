...
#include "FeSDLUtils.h"
#include "FeFastEvents.h"
...
void printSDLPixelFormat(const SDL_PixelFormat *f) {
	...
	if (f->palette) printSDLPalette(f->palette);
	fflush(NULL);
}

// fills struct SDL_PixelFormat with zeros
void fillSDLPixelFormat(SDL_PixelFormat *f, Uint32 bitspp, Uint32 bytespp) {
	if (! f) return;
	memset(f, 0x00, sizeof(SDL_PixelFormat));
...
void printSDLInitFlags(const Uint32 f) {
	printf("\nSDL_InitFlags (%08x)\n", f);
	int i = 0; 
	for (; i < numInitFlags; i++) {
		if (f & initFlags[i].value)
...
void printSDLSurfaceFlags(const Uint32 f) {
	...
	if ((! (f & SDL_HWSURFACE)) && (! (f & SDL_OPENGL)))
		printf(" %08x %s\n", surfaceFlags[0].value, surfaceFlags[0].name);
	int i = 0;
	for (; i < numSurfaceFlags; i++) {
		if (f & surfaceFlags[i].value)
	...
	switch (e->type) {
		case SDL_ACTIVEEVENT:			/* Application loses/gains visibility */
			...
			for (i = 0; i < numAppFlags; i++) {
				if (appFlags[i].value & e->active.state)
...
void printSDLOpenGLAttrs() {
	...
	if (! SDL_GL_GetAttribute(SDL_GL_BUFFER_SIZE, &value))
	...
	if (! SDL_GL_GetAttribute(SDL_GL_ACCUM_RED_SIZE, &value))
		printf("%s=%3d\n", "SDL_GL_ACCUM_RED    ", value);

	if (! SDL_GL_GetAttribute(SDL_GL_ACCUM_GREEN_SIZE, &value))
		printf("%s=%3d\n", "SDL_GL_ACCUM_GREEN  ", value);

	if (! SDL_GL_GetAttribute(SDL_GL_ACCUM_BLUE_SIZE, &value))
		printf("%s=%3d\n", "SDL_GL_ACCUM_BLUE   ", value);

	if (! SDL_GL_GetAttribute(SDL_GL_ACCUM_ALPHA_SIZE, &value))
...
char *mySDL_Init(Uint32 flags) {
	if (SDL_Init(flags) == -1) return SDL_GetError();

	//if (SDLNet_Init() == -1)   return SDLNet_GetError();

	if (FE_Init() == -1)       return FE_GetError();
