...
static __inline__ void setError(char *err) { error = err; }

inline char *FE_GetError() { return error; }
...
//!----------------------------------------
//!
//! Replacement for SDL_PushEvents();
//!
...
int FE_WaitEvent(SDL_Event *event) {
	...
	while ((val = SDL_PollEvent(event)) <= 0)
		SDL_CondWait(eventWait, eventLock);
...
//!
//!
//! Timer which is called every 10ms

static Uint32 timerCallback(Uint32 interval, void *param) {
	SDL_CondBroadcast(eventWait);	/* Wake thread either Waiting for event or Pushing event */
	return interval;
}

//!----------------------------------------
//!
//! FastEvent initialize
//! on Linux call: SDL_Init(...|SDL_INIT_EVENTTHREAD) before FE_Init()

int FE_Init() {
	if (! (SDL_INIT_TIMER & SDL_WasInit(SDL_INIT_TIMER)))
...
//!
//!
//! Call this before SDL_Quit() on program ending
