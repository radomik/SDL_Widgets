...
extern "C" {
	...
	inline const char* coUtils_getTrueStr();
	inline const char* coUtils_getFalseStr();
	inline const char* coUtils_getNullStr();
	inline const char* coUtils_getVerticalStr();
	inline const char* coUtils_getHorizontalStr();

	#define CO_TRUE_STR (coUtils_getTrueStr())
	#define CO_FALSE_STR (coUtils_getFalseStr())
	#define CO_NULL_STR (coUtils_getNullStr())
	#define CO_VERTICAL_STR (coUtils_getVerticalStr())
	#define CO_HORIZONTAL_STR (coUtils_getHorizontalStr())

	#define coSbool(VAL) ((VAL)?((char*)coUtils_getTrueStr()):((char*)coUtils_getFalseStr()))
	#define coSlayout(LAYOUT) (((LAYOUT)==VERTICAL)?(coUtils_getVerticalStr()):(coUtils_getHorizontalStr()))
	#define coPointInside(X, Y, XMIN, XMAX, YMIN, YMAX) (((X)>=(XMIN))&&((Y)>=(YMIN))&&((X)<=(XMAX))&&((Y)<=(YMAX)))
	#define coCstr(CONST_PCHAR) ((CONST_PCHAR)?((char*)CONST_PCHAR):((char*)coUtils_getNullStr()))
	...
	inline b8 coFileExist(const char *path);

	// Sleep given amount of miliseconds
	inline void coSleep(u32 milis);
	...
	inline void coFill4fv(float *tab, float v0, float v1, float v2, float v3);
	inline void coFill3fv(float *tab, float v0, float v1, float v2);

	void coRgbaToArray(u32 rgba, u8 *v);
	inline char* coCopyCstr(const char *cstr); // newly malloced and copied char* is returned
								  // NULL on error
