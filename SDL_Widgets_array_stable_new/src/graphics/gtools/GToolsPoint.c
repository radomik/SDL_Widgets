...
#define LOWER_SYM_DEF			"<"
#define GREATER_SYM_DEF			">"
#define LOWER_EQUAL_SYM_DEF		"≤"
#define GREATER_EQUAL_SYM_DEF	"≥"
#define EQUAL_SYM_DEF			"="
#define NOT_EQUAL_SYM_DEF		"≠"

const char *LOWER_SYM 			= LOWER_SYM_DEF;
const char *GREATER_SYM 		= GREATER_SYM_DEF;
const char *LOWER_EQUAL_SYM		= LOWER_EQUAL_SYM_DEF;
const char *GREATER_EQUAL_SYM	= GREATER_EQUAL_SYM_DEF;
const char *EQUAL_SYM			= EQUAL_SYM_DEF;
const char *NOT_EQUAL_SYM		= NOT_EQUAL_SYM_DEF;

const char *COND_SYM[] = {	//! same order as in GToolsPoint.h
	LOWER_SYM_DEF, GREATER_SYM_DEF, LOWER_EQUAL_SYM_DEF, GREATER_EQUAL_SYM_DEF, EQUAL_SYM_DEF, NOT_EQUAL_SYM_DEF
};
										...
										float img_mul, s16 img_off, b8 allow_overflow) 
{
	...
	u32  pitches = 0, pitch = imgsrc->pitch;
	u32  wb = imgsrc->w * Bpp; 				// count of bytes of image per line		
	
	const u8 *pixels_src    = imgsrc->pixels;
	u8 *pixels_dst     		= imgdst->pixels;
	u8 *pixels_dst_end 		= (u8*) imgdst->pixels + pitch * imgdst->h;
		...
		if (Bpp == 4) { // assume ARGB
			...
			while (pixels_dst < pixels_dst_end) {
				...
				if (allow_overflow) {
					...
					r = ((rr<0x00)?0x00:((rr>0xFF)?0xFF:((u32)rr)));
					g = ((gg<0x00)?0x00:((gg>0xFF)?0xFF:((u32)gg)));
					b = ((bb<0x00)?0x00:((bb>0xFF)?0xFF:((u32)bb)));
				}
...
u8 GToolsPoint_desaturize_std(const SDL_Surface *imgsrc, SDL_Surface *imgdst) {
	...
	if (Bpp == 3) {
		while (pixels_dst < pixels_dst_end) {
			...
			memset(pixels_dst, r, Bpp);		// 3 * sizeof(u8) == 3
...
u8 GToolsPoint_create_histograms(const SDL_Surface *image, u32 **colors) {
	...
	u8 Bpp = image->format->BytesPerPixel;
	if (Check_Bpp(Bpp)) {
		fprintf(stderr, "GToolsPoint_create_histograms() > Unsupported BytesPerPixel format %d\n", Bpp);
		return ERROR_UNSUPPORTED_BPP;
	}
	u32  pitches = 0, pitch = image->pitch;
	u32  wb = image->w * Bpp;
	u32  col, r, g, b;
	const u8 	*pixels_src     = image->pixels;
	const u8 	*pixels_src_end = pixels_src + image->h * pitch;
	
	memset(colors[0], 0, 256*sizeof(u32));
	memset(colors[1], 0, 256*sizeof(u32));
	memset(colors[2], 0, 256*sizeof(u32));
...
u8 GToolsPoint_calculate_binary_threshold(const u32 *histogram, u64 *threshold, b8 t_algo) {
	u64 tmp_t=0, tmp_t_2=0, t=0, t_cz, t_b, tmp_t_cz, tmp_t_b, tt;
	u32 k, m;
...
u8 GToolsPoint_negative(SDL_Surface *imgdst) {
	...
	u8 Bpp = imgdst->format->BytesPerPixel;
	if (Check_Bpp(Bpp)) {
		fprintf(stderr, "GToolsPoint_negative() > Unsupported BytesPerPixel format %d\n", Bpp);
		return ERROR_UNSUPPORTED_BPP;
	}
	u32  pitches = 0, pitch = imgdst->pitch;
	u32  wb = imgdst->w * Bpp; 				// count of bytes of image per line	
	u32  col, r, g, b, a;
	u8 *pixels_dst     = (u8*) imgdst->pixels;
	u8 *pixels_dst_end = (u8*) imgdst->pixels + pitch * imgdst->h;
	if (SDL_MUSTLOCK(imgdst)) SDL_LockSurface(imgdst);
	if (Bpp == 3) {
		while (pixels_dst < pixels_dst_end) {
...
u8 GToolsPoint_histEqualize(SDL_Surface *imgdst, u32 **colors) {
	...
	u8 *pixels_dst     = (u8*) imgdst->pixels;
	u8 *pixels_dst_end = (u8*) imgdst->pixels + pitch * imgdst->h;
	u32  **dystrybuanta = malloc(3 * sizeof(u32*));
	
	dystrybuanta[0] = calloc(256, sizeof(u32));
	dystrybuanta[1] = calloc(256, sizeof(u32));
	dystrybuanta[2] = calloc(256, sizeof(u32));
	
	u64 	t[3];
	u8 	min_col[3], channel;
	u64	sum[3]   = {0, 0, 0};
	b8 	found[3] = {false, false, false};
	...
	*/
	...
	memset(colors[0], 0x00, 256*sizeof(u32));
	memset(colors[1], 0x00, 256*sizeof(u32));
	memset(colors[2], 0x00, 256*sizeof(u32));
	
	u32  wh = imgdst->w * imgdst->h; 
	double v[3], licz[3], 
	...
	while (pixels_dst < pixels_dst_end) {
		...
		for (channel = 0; channel < 3; channel++) {
			...
			if ((v[channel] - (u8)v[channel]) > 0.50) v[channel] += 1.0;
			
			col[channel] = (u8)v[channel];
			//fprintf(stderr, "\tcol[%d] = %u\n", channel, col[channel]);
	...
	pitches = 0; p=0;
	pixels_dst     = (u8*) imgdst->pixels;
	pixels_dst_end = (u8*) imgdst->pixels + pitch * imgdst->h;
	while (pixels_dst < pixels_dst_end) {
		if (pitches == wb) {	// end of image line
			while(pitches < pitch) { pitches++; pixels_dst++;}	// skip line separating bytes
			pitches = 0;	// index of first byte in new line
...
u8 GToolsPoint_replace_pixels(	const SDL_Surface *imgsrc, SDL_Surface *imgdst, 
	...
	u32  wb = imgsrc->w * Bpp; 				// count of bytes of image per line	
	u32  col, r, g, b, a;
	const u32	r_rep = ((u32)new_color[0]) << imgsrc->format->Rshift;
	const u32	g_rep = ((u32)new_color[1]) << imgsrc->format->Gshift;
	const u32	b_rep = ((u32)new_color[2]) << imgsrc->format->Bshift;
	const u8 	*pixels_src 	 = imgsrc->pixels;
	u8 			*pixels_dst    	 = imgdst->pixels;
	u8 			*pixels_dst_end  = (u8*) imgdst->pixels + pitch * imgdst->h;
	u8 min, max;
	...
	while (pixels_dst < pixels_dst_end) {
		...
		col = *((const u32*)pixels_src);
		
		r   = (col & imgsrc->format->Rmask)>>imgsrc->format->Rshift;
		g   = (col & imgsrc->format->Gmask)>>imgsrc->format->Gshift;
		b   = (col & imgsrc->format->Bmask)>>imgsrc->format->Bshift;
		
		min = (u8)((r < g) ? ((b<r) ? b : r) : ((b<g) ? b : g));
		max = (u8)((r > g) ? ((b>r) ? b : r) : ((b>g) ? b : g));
		...
		if (res) {
			if (Bpp == 3) {
				col = r_rep|g_rep|b_rep; 
				Static_set24bit_pixel(pixels_dst, col);
			}
			else {
				if (Bpp == 4) {
					a = col & imgsrc->format->Amask;
					col = r_rep|g_rep|b_rep|a; 
					*((u32*)pixels_dst) = col;
				}
			}
		}
...
u8 GToolsPoint_histogram_stretching(const SDL_Surface *imgsrc, SDL_Surface *imgdst,
	...
	u8 j, i, Bpp = imgsrc->format->BytesPerPixel;
	if (Check_Bpp(Bpp)) {
	...
	while (pixels_dst < pixels_dst_end) {
		...
		for (j = 0; j < 3; j++)	{			// foreach channel 
			for (i = 1; i < size; i++) {	// find appropiate range
				if (c[j] > (u32)x[i]) {  continue; }
				if (c[j] == (u32)x[i-1]) { c[j] = (u32)y[i-1];  break; }
				else {
					if (c[j] == (u32)x[i]) { c[j] = (u32)y[i]; break; }
					else {
		...
		if (Bpp == 3) {
			...
			if (Bpp == 4) {
				col = c[0]|c[1]|c[2]|c[3]; *((u32*)pixels_dst) = col;
			}
		}
		pixels_src += Bpp;
		pixels_dst += Bpp;
