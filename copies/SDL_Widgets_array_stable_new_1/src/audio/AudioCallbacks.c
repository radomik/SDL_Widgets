...
void button_freq1_change(Widget *sender, Screen *screen) {
	...
	AudioTest	*audiotest 	= (AudioTest*) sender->cparam[1];
	if (sender->id) {
...
void button_source_change(Widget *sender, Screen *screen) {
	...
	usint	*audio_current_index = (usint*)sender->cparam[2];
	usint	*cnt_audio = (usint*)sender->cparam[3];
	...
	bool _play  = audio[*audio_current_index]->play;
	bool _pause = audio[*audio_current_index]->pause;
	if (sender->id) {
		if (*audio_current_index != 0) {
			Audio_remove(audio[*audio_current_index]);
			(*audio_current_index)--;
		}
		else return;
	}
	...
	if (_play) Audio_play(audio[*audio_current_index], _pause);
	sprintf((label->text_block.text+22), "%20s", audio[*audio_current_index]->name);
	Label_refresh(label);
	screen->need_reload = true;
}
...
void image_mrelease(Widget *sender, Screen *screen) {
	...
	AudioSinus 	*audiosinus = (AudioSinus*) sender->vparam[1];
...
void button_audio_from_graph_type_change(Widget *sender, Screen *screen) {
	Label *label 		= (Label*) sender->cparam[0];
	AudioFromGraph *afg = (AudioFromGraph*)sender->cparam[1];
...
void button_audio_from_graph_clear(Widget *sender, Screen *screen) {
	WaveDrawBox *wdb = (WaveDrawBox*) sender->vparam[0];
