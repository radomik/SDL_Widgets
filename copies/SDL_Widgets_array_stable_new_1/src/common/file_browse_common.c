							...
							) 
{
	...
	struct dirent 	*dir;
	int 			t;
	uint 			fcount = 0,					// count of subfiles(subfolders) in current folder
					i, 
	...
	wt = &(tb->widget);
	wt->click_handler = filename_textblock_click_handle;
	wt->void_parameter = click_parameter;
	wt->mevent = true;
	wt->id = 0;
	*nameHeight = wt->pos.h;	// nameHeight will be the maximum of heights of all filenames
