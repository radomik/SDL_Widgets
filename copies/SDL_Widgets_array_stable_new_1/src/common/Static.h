 ...
 */
	...
	/*
	uint Static_rgbFromColor(SDL_Color color) {
		uint r = (color.r << 16);
		uint g = (color.g << 8);
		uint b = (color.b);
		return r|g|b;
	...
	*/
	#define Static_rgbFromColor(_STRUCT_SDL_COLOR, _PTR_STRUCT_SDL_PIXELFORMAT) ( ( ((uint)(_STRUCT_SDL_COLOR).r) << (_PTR_STRUCT_SDL_PIXELFORMAT)->Rshift )|( ((uint)(_STRUCT_SDL_COLOR).g) << (_PTR_STRUCT_SDL_PIXELFORMAT)->Gshift )|( ((uint)(_STRUCT_SDL_COLOR).b) << (_PTR_STRUCT_SDL_PIXELFORMAT)->Bshift ) )
	...
	bool Static_fileExist(const char *path);
